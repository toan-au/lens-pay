<template>
  <div class="flex flex-col gap-6">
    <div class="flex items-center justify-between">
      ...
      <button @click="showForm = !showForm" :aria-expanded="showForm" aria-controls="new-customer-form" class="btn-primary">
        {{ showForm ? 'Cancel' : '+ New Customer' }}
      </button>
    </div>

    <div v-if="showForm" id="new-customer-form" class="bg-white rounded-xl border border-gray-200 p-6">
      <form @submit.prevent="handleCreate" class="flex flex-col gap-4">
        ...
        <div class="flex gap-4">
          <div class="flex flex-col gap-1 flex-1">
            ...
            <input :id="nameId" v-model="form.name" type="text" required placeholder="Jane Doe" class="input" />
          </div>
          <div class="flex flex-col gap-1 flex-1">
            <label :for="emailId" class="text-sm font-medium">Email</label>
            <input :id="emailId" v-model="form.email" type="email" required placeholder="jane@example.com" class="input" />
          </div>
        </div>
        <p v-if="createError" role="alert" class="text-sm text-red-500">{{ createError }}</p>
    ...
    <ResourceTable :loading="loading" :is-empty="customers.length === 0" :cols="4" empty-text="No customers yet." caption="Customers">
      <template #head>
        <th scope="col" class="text-left px-4 py-3 font-medium text-gray-500">Name</th>
        <th scope="col" class="text-left px-4 py-3 font-medium text-gray-500">Email</th>
        <th scope="col" class="text-left px-4 py-3 font-medium text-gray-500">ID</th>
        <th scope="col" class="text-left px-4 py-3 font-medium text-gray-500">Created</th>
      </template>
      <template #body>
        <tr
          v-for="customer in customers"
          :key="customer.uid"
          @click="rowClick($event, `/customers/${customer.uid}`)"
          class="border-t border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors"
        >
          <td class="px-4 py-3 font-medium">
            <RouterLink :to="`/customers/${customer.uid}`" class="hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">
              {{ customer.name }}
            </RouterLink>
          </td>
          <td class="px-4 py-3 text-gray-500">{{ customer.email }}</td>
          <td class="px-4 py-3 font-mono text-xs text-gray-500">{{ customer.uid.slice(0, 16) }}</td>
