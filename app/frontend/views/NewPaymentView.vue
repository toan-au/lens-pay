<template>
  <div class="flex flex-col gap-6 max-w-md">
    <div class="flex items-center gap-3">
      <button @click="router.back()" class="btn-ghost text-xs cursor-pointer"><span aria-hidden="true">←</span> Back</button>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 text-sm text-amber-800">
      ...
      See the <a href="/api-docs" target="_blank" rel="noopener" class="underline font-medium hover:text-amber-900">API documentation<span class="sr-only"> (opens in a new tab)</span></a> to call this endpoint directly.
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
      <form @submit.prevent="handleSubmit" class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-1">
          <label :for="amountId" class="text-sm font-medium">Amount</label>
          <div class="flex gap-2">
            <input
              ...
              :aria-describedby="`${currencyId} ${amountHintId}`"
              class="input flex-1"
            />
            <span :id="currencyId" class="input bg-gray-50 text-gray-500 min-w-16 text-center">
              {{ merchantStore.merchant?.currency ?? '...' }}
            </span>
          </div>
          <p :id="amountHintId">
            <span v-if="form.amount" class="text-sm font-medium text-indigo-600">
              = {{ formatAmount(toMinorUnits(form.amount, currency), currency) }}
            </span>
            <span v-else class="text-xs text-gray-500">{{ currencyHint }}</span>
          </p>
        </div>

        <div class="flex flex-col gap-1">
          <span :id="methodLabelId" class="text-sm font-medium">Payment Method</span>
          <div role="group" :aria-labelledby="methodLabelId" class="grid grid-cols-3 gap-2">
            <button
              ...
              class="rounded-lg border px-3 py-2 text-sm cursor-pointer transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500"
          ...
          <p class="text-xs text-gray-500">{{ methodHint }}</p>
        </div>

        <div class="flex flex-col gap-1">
          <label :for="keyId" class="text-sm font-medium">Idempotency Key</label>
          <div class="flex gap-2">
            <input :id="keyId" v-model="form.idempotency_key" type="text" required class="input flex-1 font-mono text-xs" />
            <button type="button" @click="regenerateKey" class="btn-ghost text-xs whitespace-nowrap cursor-pointer">Regenerate</button>
          </div>
          <p class="text-xs text-gray-500">Prevents duplicate payments on retries.</p>
        </div>
        ...
        <div class="flex flex-col gap-2">
          <div class="flex items-center justify-between">
            <span :id="metadataLabelId" class="text-sm font-medium">Metadata</span>
            <button type="button" @click="addMetadataRow" class="text-xs text-indigo-600 hover:text-indigo-800 cursor-pointer">+ Add field</button>
          </div>
          <p class="text-xs text-gray-500">Attach arbitrary key-value data to this payment — useful for linking to an order ID, customer, or any internal reference.</p>
          <div v-if="metadataRows.length > 0" class="flex flex-col gap-2" role="group" :aria-labelledby="metadataLabelId">
...
const metadataRows = reactive<{ id: string; key: string; value: string }[]>([
  { id: crypto.randomUUID(), key: 'order_id', value: 'order_demo_001' },
])

function addMetadataRow() { metadataRows.push({ id: crypto.randomUUID(), key: '', value: '' }) }
