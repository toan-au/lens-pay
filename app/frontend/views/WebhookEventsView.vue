<template>
  <div class="flex flex-col gap-6">
    ...
    <ResourceTable :loading="loading" :is-empty="events.length === 0" :cols="3" empty-text="No webhook events yet." caption="Webhook events">
      <template #head>
        <th scope="col" class="text-left px-4 py-3 font-medium text-gray-500">Event</th>
      ...
      <template #body>
        <template v-for="event in events" :key="event.id">
          ...
          >
            <td class="px-4 py-3 font-mono text-xs">
              <button
                ...
                @click.stop="toggleRow(event.id)"
                class="flex items-center gap-2 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500"
              >
                <span aria-hidden="true">{{ expandedId === event.id ? '▾' : '▸' }}</span>
                {{ event.event_type }}
              </button>
            </td>
          ...
          <tr v-if="expandedId === event.id" :id="`event-payload-${event.id}`" class="border-t border-gray-100 bg-gray-50">
