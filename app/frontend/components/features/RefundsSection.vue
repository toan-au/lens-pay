    ...
    <div v-if="refunds.length > 0" class="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
      ...
      >
        <div class="flex flex-col gap-0.5">
          ...
          <span class="text-xs text-gray-500">{{ formatDate(refund.created_at) }}</span>
        </div>
        <StatusBadge :status="refund.status" />
      </div>
    </div>

    <p v-else class="text-sm text-gray-500">No refunds yet.</p>
        ...
        <form @submit.prevent="handle" class="flex flex-col gap-3">
          <div class="flex flex-col gap-1">
            <label :for="amountId" class="text-sm font-medium">
              Amount
              <span class="text-gray-500 font-normal">— max {{ formatAmount(remainingAmount, currency) }}</span>
            </label>
